...
Manifest YAML fields:
  ...
  NEVER set captions: true unless the person asked for captions — they are off by default because a caption makes a viewer read instead of watch. Write a caption on each scene anyway: it names the beat in the proof log and the stills. NEVER set viewport. The preset decides the size and every take at a preset comes out identical; setting viewport makes this demo a different shape from every other one and players letterbox it. Same for cursor, chrome, background, fps, crf and captions — the preset handles all of it.
...
Steps (each may have pauseAfter ms, waitFor selector, timeout ms):
  ...
  {action: select, selector, value}   choose in a <select> — value or the visible label, whichever the page uses
  {action: wait, ms}
